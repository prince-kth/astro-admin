...
const monthlyFortuneSchema = z.object({
  ...
  focus_areas: z.object({
    ...
    lucky_symbols: z.string().describe("Lucky symbols, <strong>lucky numbers</strong>, and lucky colors for the month must be at least 350 words in bullet points not in paragraph format"),
    remedies: z.string().describe("Recommended remedies for the month must be at least 200 words and should include donation, chanting, service and affirmations only. Do not suggest any Gemstone, Rudraksh,crystle,yantra etc. Feeding [animal,birds,fish,...etc]")
  })
});
