...
export async function POST(req: NextRequest) {
  try {
    ...
    const [basicAnalysis, lucky13Analysis, spiritualAndFamilyAnalysis] = await Promise.all([
      generateObject({
        model: openai('gpt-4o'),
      ...
      //   - Lucky Sounds for Car Brand: Provide 4-5 two-letter sounds based on birth number's ruling planet (e.g., "Ma", "Me", "Mi", "Mu", "Mo")
      //   - Lucky Sounds for Bank: Provide 4-5 two-letter sounds based on destiny number's ruling planet (e.g., "Ga", "Ge", "Gi", "Go", "Gu")
      ...
      generateObject({
        model: openai('gpt-4o'),
        prompt: `Analyze the spiritual path and family dynamics for ${name}.
        Include life purpose, karmic analysis, and family relationships.

        Birth Chart Details:
        ${JSON.stringify(kundli_data, null, 2)}`,
        schema: z.object({
          spirituality_and_growth: z.object({
            dharma_and_life_purpose: z.string(),
            karmic_analysis: z.string(),
            guru_influence: z.string(),
          }),
          family_and_domestic_life: z.object({
            parenting_style: z.string(),
            family_relations: z.string(),
            siblings: z.string(),
          }),
        }),
        temperature: 0.7,
